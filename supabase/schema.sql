...
-- Append-only snapshot taken on every member_renewals write (see
-- updateMemberRenewal in mockMembershipStore.js) — member_renewals only
-- ever holds one current row per member, so without this, a past term's
-- record would be gone the moment a member gets renewed for the next
-- one. Powers the Renewal Management page's separate "Previous Terms"
-- view instead of mixing historical and current data in one table.
create table if not exists member_renewal_history (
  id bigint generated always as identity primary key,
  email text not null,
  member_name text not null,
...
alter table member_renewal_history enable row level security;

drop policy if exists "renewal history authenticated select" on member_renewal_history;
create policy "renewal history authenticated select" on member_renewal_history
  for select to authenticated using (true);
drop policy if exists "renewal history treasurer or president insert" on member_renewal_history;
create policy "renewal history treasurer or president insert" on member_renewal_history
  for insert to authenticated
  with check (
    exists (
      select 1 from excom_appointments
...
grant select, insert on member_renewal_history to authenticated;
grant usage, select on all sequences in schema public to authenticated;
