...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-cream">
      ...
      {/* Hero — text on a plain background (no overlay tricks needed)
          and the 4 photos in their own tidy grid, instead of both
        ...
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <a
                href="#find-club"
                className="rounded-full bg-primary px-6 py-3 text-sm font-semibold text-cream shadow-md shadow-primary/20 transition hover:bg-primary-dark"
              >
                Find Your Club
              </a>
              <a
          ...
          <div className="grid grid-cols-2 gap-4">
            {heroPhotos.map((photo) => (
              <div
                key={photo.src}
                className="aspect-square overflow-hidden rounded-3xl shadow-lg shadow-primary/10"
              >
                <img
                  src={photo.src}
                  alt={photo.alt}
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="grid gap-5 sm:grid-cols-3">
          {features.map(({ icon: Icon, title, description }) => (
            <div key={title} className="rounded-3xl bg-white p-6 shadow-sm shadow-ink/5">
